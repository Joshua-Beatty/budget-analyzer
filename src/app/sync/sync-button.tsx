...
type Props = {
  /** Whether an Access URL is stored; the button is disabled without one. */
  hasToken: boolean;
  /** Epoch ms of the last successful sync, if any. */
  initialLastSyncAt?: number;
};

export function SyncButton({ hasToken, initialLastSyncAt }: Props) {
  ...
  return (
    <div className="flex w-full flex-col gap-4">
      ...
      {promptOpen ? (
        <div className="flex w-full max-w-sm flex-col gap-3 rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-950">
          <label
          ...
          <p className="text-xs text-zinc-500 dark:text-zinc-400">
            Fetched in 45-day windows. The SimpleFIN Bridge may serve fewer
